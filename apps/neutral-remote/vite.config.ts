...
export default defineConfig(({ command }) => ({
  ...
  cacheDir: fileURLToPath(
    new URL("../../node_modules/.vite/neutral-remote", import.meta.url),
  ),
  plugins: [react(), federation(createFederationConfig(command === "serve"))],
  server: { origin: "http://localhost:4201", port: 4201, strictPort: true },
  base: "http://localhost:4201",
  build: {
    target: "chrome89",
    outDir: fileURLToPath(
      new URL("../../dist/apps/neutral-remote", import.meta.url),
    ),
    emptyOutDir: true,
  },
  resolve: {
