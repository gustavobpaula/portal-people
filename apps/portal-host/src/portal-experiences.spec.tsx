...
const capabilities: PlatformCapabilities = {
  navigate: vi.fn(),
  context: { correlationId: 'correlation-core', locale: 'pt-BR', platform: 'web' },
  telemetry: { track },
  flags: {},
...
function renderHome(initialEntry = '/') {
  track.mockClear();
  return render(<MemoryRouter initialEntries={[initialEntry]}><App createCapabilities={() => capabilities} /><LocationProbe /></MemoryRouter>);
}
...
describe('portal experiences', () => {
  ...
  it('keeps submitted search results on the home route and restores them from q', async () => {
    ...
    expect(await screen.findByTestId('location')).toHaveTextContent('/?q=PLATAFORMA');
    expect(await screen.findByRole('heading', { name: 'Resultados para “PLATAFORMA”' })).toBeInTheDocument();
    cleanup();
  ...
  it('shows a clear action for an empty search and restores the catalog', async () => {
    ...
    expect(await screen.findByRole('heading', { name: 'Nenhum resultado encontrado' })).toBeInTheDocument();
  ...
  it('opens notifications from the header and persists reads for the current browser session only', async () => {
    ...
    expect(await screen.findByTestId('location')).toHaveTextContent('/notificacoes');
    const unreadNotification = await screen.findByRole('button', { name: 'Portal atualizado, não lida' });
    expect(unreadNotification.className).toContain('notificationItemUnread');
    unreadNotification.focus();
    await user.keyboard('{Enter}');
    expect(screen.getByText('Todas as notificações foram lidas')).toBeInTheDocument();
    cleanup();
    renderHome('/notificacoes');
    expect(await screen.findByText('Todas as notificações foram lidas')).toBeInTheDocument();
  ...
  it('retries only the Produtos request', async () => {
    server.use(http.get('/api/portal/catalog', () => new HttpResponse(null, { status: 500 })));
    const user = userEvent.setup();
    renderHome();
    expect(await screen.findByRole('heading', { name: 'Não foi possível carregar os Produtos' })).toBeInTheDocument();
  ...
  it('renders distinct loading and empty states for Produtos and notifications', async () => {
    server.use(
      ...
      http.get('/api/portal/notifications', () => HttpResponse.json({ items: [] }))
    );
    renderHome();
    expect(screen.getByRole('status', { name: 'Carregando Produtos' })).toBeInTheDocument();
    expect(await screen.findByRole('heading', { name: 'Nenhum produto disponível' })).toBeInTheDocument();
    cleanup();
    renderHome('/notificacoes');
    expect(await screen.findByRole('heading', { name: 'Nenhuma notificação' })).toBeInTheDocument();
  });

  it('retries notification loading without changing the session state', async () => {
    server.use(http.get('/api/portal/notifications', () => new HttpResponse(null, { status: 500 })));
    const user = userEvent.setup();
    renderHome('/notificacoes');
    expect(await screen.findByRole('heading', { name: 'Não foi possível carregar as notificações' })).toBeInTheDocument();
    server.resetHandlers();
    await user.click(screen.getByRole('button', { name: 'Tentar novamente' }));
    expect(await screen.findByRole('button', { name: 'Portal atualizado, não lida' })).toBeInTheDocument();
  });
  ...
  it('emits sanitized search and notification telemetry', async () => {
    ...
    await user.click(await screen.findByRole('button', { name: 'Portal atualizado, não lida' }));
    ...
    expect(track).toHaveBeenCalledWith(expect.objectContaining({ name: 'portal.core.search.submitted', properties: expect.objectContaining({ route: '/', platform: 'web', correlationId: 'correlation-core' }) }));
    expect(track).toHaveBeenCalledWith(expect.objectContaining({ name: 'portal.core.notifications.opened', properties: expect.objectContaining({ route: '/notificacoes', platform: 'web', correlationId: 'correlation-core' }) }));
    expect(track).toHaveBeenCalledWith(expect.objectContaining({ name: 'portal.core.notification.read', properties: expect.objectContaining({ route: '/notificacoes' }) }));
  });
});
