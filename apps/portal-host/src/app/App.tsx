...
import { createWebCapabilities, loadFederatedJourney, resolveJourneyRegistry } from "@portal/platform-runtime";
...
import type { AppProps, ExternalManifest, FederatedManifest } from "./types";
...
function ShellContent({ registryData, loadJourney = loadFederatedJourney, createCapabilities = createWebCapabilities, portalBffClient: bffClient = portalBffClient, externalOrigins = EXTERNAL_WEB_ALLOWED_ORIGINS, checkExternalAvailability = isExternalJourneyAvailable, navigateExternal = (destination) => window.location.assign(destination) }: AppProps & { registryData: unknown }) {
  const navigate = useNavigate();
  const [platform] = useState(() => createCapabilities({ navigate: (path) => navigate(path), telemetry: { track: (event) => console.info("portal-event", event.name, event.properties) } }));
  const resolution = useMemo(() => resolveJourneyRegistry(registryData), [registryData]);
  const journeys = resolution.journeys.filter((manifest): manifest is FederatedManifest | ExternalManifest => manifest.strategy === "federated-module" || manifest.strategy === "external-web");
  ...
  return <div className={styles.shell}>
    ...
    <main className={styles.main}>
      ...
      <Routes>
        ...
        {journeys.map((manifest) => <Route key={manifest.id} path={`${manifest.route}/*`} element={manifest.strategy === "federated-module" ? <FederatedJourneySlot manifest={manifest} platform={platform} loadJourney={loadJourney} /> : <ExternalJourneySlot manifest={manifest} platform={platform} allowedOrigins={externalOrigins} checkExternalAvailability={checkExternalAvailability} navigateExternal={navigateExternal} />} />)}
