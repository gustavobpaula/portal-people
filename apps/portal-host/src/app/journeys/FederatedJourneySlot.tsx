...
export function FederatedJourneySlot({
  ...
  useEffect(() => {
    ...
    void loadJourney(manifest).then((next) => {
      if (!active) return;
      setResult(next);
      trackJourney(platform, "portal.journey.load.duration", manifest, {
        result: next.status === "ready" ? "success" : "failure",
        durationMs: Math.round(performance.now() - startedAt),
      });
      trackJourney(
        platform,
        next.status === "ready"
          ? "portal.journey.load.succeeded"
          : "portal.journey.load.failed",
        manifest,
        next.status === "ready" ? {} : { reason: next.reason },
      );
    });
    return () => {
      active = false;
  ...
  if (!result)
    return (
      <div className={styles.loading}>
        <Spinner size="lg" label={`Carregando ${label(manifest)}`} />
      </div>
    );
