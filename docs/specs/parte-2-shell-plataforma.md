...
- **FR-2:** O shell deve resolver uma coleção de manifestos já selecionados e derivar dela as rotas principais, sem importar implementações de domínio, exigir alteração da configuração interna para cada nova jornada ou aplicar regras de audiência e release. A descoberta listada das jornadas ocorre por Produtos, conforme `docs/specs/parte-2-experiencias-transversais.md`.
...
- O registro e os destinos permanecem como fixtures locais, determinísticas e já resolvidas neste incremento.
...
- A coleção local de manifestos representa a resposta já resolvida de um futuro Journey Registry, sem simular usuários, feature flags ou canais de release.
...
- Execução de `external-web` e `native-route`, bridge nativa, Journey Registry remoto, seleção de audiência, release, rollout, rollback e infraestrutura.
