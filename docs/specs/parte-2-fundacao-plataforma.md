...
- **FR-4:** O contrato `JourneyManifest` deve representar `federated-module`, `external-web` e `native-route`, com identidade, rota, versão, compatibilidade, owner, observabilidade e rollout.
...
- **AC-5 [FR-4, FR-5]:** Schemas e testes rejeitam estratégia, versão, rota ou capability inválida e não expõem tokens ou estado de negócio.
...
- O manifesto é local e determinístico neste marco.
...
- Shell funcional, Benefícios, Férias, backend, registry remoto e deploy.
