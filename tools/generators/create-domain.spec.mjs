...
async function createWorkspace(domains = []) {
  ...
  await writeFile(
    join(workspaceRoot, "tools/domain-governance.json"),
...
describe("golden path", () => {
  ...
  it("materializes a complete federated remote outside the shell", async () => {
    ...
    const destination = await createDomain({
      name: "nova-jornada",
      displayName: "Nova jornada",
      port: 4301,
      workspaceRoot,
    });
    expect(
      await readFile(
        join(workspaceRoot, "journeys", "nova-jornada", "manifest.json"),
        "utf8",
      ),
    ).toContain("http://localhost:4301/mf-manifest.json");
    const viteConfig = await readFile(
      join(destination, "vite.config.ts"),
      "utf8",
    );
    expect(viteConfig).toContain(
      'federation(createFederationConfig(command === "serve"))',
    );
    ...
    expect(
      await readFile(join(destination, "src/app/Journey.spec.tsx"), "utf8"),
    ).toContain("Nova jornada");
    expect(
      await readFile(join(destination, "src/app/Journey.spec.tsx"), "utf8"),
    ).toContain('import { expect, it } from "vitest"');
    expect(
      await readFile(join(destination, "src/app/Journey.tsx"), "utf8"),
    ).not.toContain("@ts-nocheck");
    expect(await readFile(join(destination, "src/main.tsx"), "utf8")).toContain(
      '"./app/Journey"',
    );
    await expect(access(join(destination, "src/domain"))).rejects.toMatchObject(
      { code: "ENOENT" },
    );
    await expect(
      access(join(destination, "src/services")),
    ).rejects.toMatchObject({ code: "ENOENT" });
    expect(
      await readFile(join(destination, "src/main.tsx"), "utf8"),
    ).not.toContain("@ts-nocheck");
    const project = JSON.parse(
      await readFile(join(destination, "project.json"), "utf8"),
    );
    ...
    ).toEqual(["legado", "nova-jornada"]);
    await expect(
      access(join(workspaceRoot, "apps/portal-host")),
    ).rejects.toMatchObject({ code: "ENOENT" });
    await expect(
      access(
        join(
          workspaceRoot,
          "apps/portal-host/src/assets/journey-registry.json",
        ),
      ),
    ).rejects.toMatchObject({ code: "ENOENT" });
  });
  it("keeps the governance catalog idempotent while preserving existing domains", async () => {
    ...
    ).toEqual(["beneficios", "ferias", "legado"]);
  });
});
